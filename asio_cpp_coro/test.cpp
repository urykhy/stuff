...
BOOST_AUTO_TEST_CASE(simple)
{
    ...
    auto sServer = asio_http::createServer({});
    auto sClient = asio_http::createClient({}); // default port 3080
    ...
    auto sFuture = boost::asio::co_spawn(
        sAsio,
        [&]() -> ba::awaitable<void> {
            auto sRequest = asio_http::Request{.method = "GET", .url = "http://localhost:3080/test"};
...
BOOST_AUTO_TEST_CASE(keep_alive)
{
    ...
    auto sServer = asio_http::createServer({});
    auto sClient = asio_http::createClient({.alive = true}); // default port 3080
