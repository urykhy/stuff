...
namespace MsgPack {
    ...
    struct BadFormat : std::runtime_error
    {
        BadFormat()
    ...
    inline void mp_store_u16(omemstream& aStream, uint16_t aValue)
    {
        aValue = htobe16(aValue);
        aStream.write(&aValue, sizeof(aValue));
    }
    inline void mp_store_u32(omemstream& aStream, uint32_t aValue)
    {
        aValue = htobe32(aValue);
        aStream.write(&aValue, sizeof(aValue));
    }
    inline void mp_store_u64(omemstream& aStream, uint64_t aValue)
    {
        aValue = htobe64(aValue);
        aStream.write(&aValue, sizeof(aValue));
    }

    inline uint8_t mp_load_u8(imemstream& aStream)
    {
        uint8_t sValue;
        aStream.read(&sValue, sizeof(sValue));
        return sValue;
    }
    inline uint64_t mp_load_u16(imemstream& aStream)
    {
        uint16_t sValue;
        aStream.read(&sValue, sizeof(sValue));
        return be16toh(sValue);
    }
    inline uint64_t mp_load_u32(imemstream& aStream)
    {
        uint32_t sValue;
        aStream.read(&sValue, sizeof(sValue));
        return be32toh(sValue);
    }
    inline uint64_t mp_load_u64(imemstream& aStream)
    {
        uint64_t sValue;
        aStream.read(&sValue, sizeof(sValue));
        return be64toh(sValue);
    }
    ...
    inline uint32_t read_map_size(imemstream& aStream)
    {
        ...
        default:
            if (sCode < FIXMAP || sCode >= FIXARRAY)
                throw BadFormat();
            return sCode & 0x0f;
        }
    }
    ...
    inline uint32_t read_array_size(imemstream& aStream)
    {
        ...
        default:
            if (sCode < FIXARRAY || sCode >= FIXSTR)
                throw BadFormat();
            return sCode & 0x0f;
        }
    }
